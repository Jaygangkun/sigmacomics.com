...
if(!isset($_SESSION['add_to_cart_session_id']) && $_SESSION['add_to_cart_session_id']== ''){
    header( 'Location:'.constant('SITEURL_FRENCH').'/order-issues');
    exit();
}else{

    $sqlNew = "SELECT count(*) as total_digital FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' AND (`type`= 'IDI' OR `type`= 'DS' OR `type`= 'PRDS' OR `type`= 'PCDS') and `language` = 'fr_FR'";
    if ($resultNew = $conn->query($sqlNew)) {
        $rowNew = $resultNew->fetch_assoc();
        if( ($rowNew['total_digital'] > 0) && ( !isset($_SESSION['user'])) ){
            header( 'Location:'.constant('SITEURL_FRENCH').'/signup');
            exit();
        }
    }


    $sqlPro = "SELECT SUM(quantity) as total_quantity,sum(price) as total_price FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' and `language` = 'fr_FR'";
            ...
            if($charge->id != ''){
                ...
                if ($conn->query($sqlOrder) === TRUE) {
                    ...
                    if( in_array('IDI',$typesArr) || in_array('DS',$typesArr) || in_array('PRDS',$typesArr) || in_array('PCDS',$typesArr) ){
                        ...
                        header( 'Location:'.constant('SITEURL_FRENCH').'/thank-you-print-2');
                    }else{
                        header( 'Location:'.constant('SITEURL_FRENCH').'/thank-you-for-order-2');
                    }
...
?>
            ...
            <div class="cartWrap">
                ...
                <div class="leftcheckout">
                    <div class="express-checkoutprt">
                        <b><p>PAYEZ RAPIDEMENT AVEC PAYPAL</p></b><br>

                        <div id="paypal-button-container"></div>
                        <div id="paypal-button" style="width: 100%"></div>
                    </div>

                    <div class="orprt">...ou payez plus lentement avec une carte de crédit</div>
                    <br>
                    <form action= "" method="post" id = "stripe_pay">
                        <div class="formPrt">
                            <span class="shipHeading"><b><?php echo (isset($content[20]))?$content[20]:'';?></b></span>
                            <div class="addressdetail">
                                <input name="first_name" id="first_name" type="text" placeholder="Prénom">
                                <input name="last_name" id="last_name" type="text" placeholder="Nom de famille">
                                <input name="address1" id="address1" placeholder="Adresse">
                                <input name="address2" id="address2" placeholder="Adresse 2">
                                <input name="city" id="city" placeholder="Ville">
                                <input name="state" id="state" placeholder="État / Province">
                                <input name="country" id="country" placeholder="De campagne">
                                <input name="zip" id="zip" placeholder="Zipper">
                            ...
                            <div class="addressdetail">
                                <div class="field-row">
                                    ...
                                    <input type="text" id="name"
                                           name="name" class="demoInputBox" placeholder="Nom du titulaire">
                                </div>
                                <div class="field-row">
                                ...
                                <div class="field-row full-width">
                                    ...
                                    <input type="text" id="card-number"
                                           name="card-number" class="demoInputBox" placeholder="Numéro de carte">
                                </div>
                                <div class="field-row full-width">
                        ...
                        <div class="reviewSection">
                            ...
                            <div class="delivery_box">
                                ...
                                <?php
                                $paypalProducts = '';
                                $sql = "SELECT * from `cart` WHERE `session_id` = '".$_SESSION['add_to_cart_session_id']."'  and `language` = 'fr_FR'";
    ...
    <script>
        function cardValidation () {
            ...
            if(valid == false) {
                $("#error-message").html("TOUS LES CHAMPS SONT REQUIS SAUF ADRESSE 2").show();
            }

            if(invalidEmail == false) {
                $("#error-message-individual").html("Email invalide. Veuillez utiliser le format correct").show();
            }else{
                $("#error-message-individual").html("").hide();
            }

            if(invalidCard == false) {
                $("#error-message-individual").html("Carte invalide. Veuillez utiliser le bon").show();
            }else{
                $("#error-message-individual").html("").hide();
            }
            ...
            onAuthorize: function (data, actions) {
                return actions.payment.execute()
                    .then(function (details) {
                        console.log(details);
                        window.location = "<?php echo constant("SITEURL_FRENCH");?>/orderDetails.php?paymentID="+
