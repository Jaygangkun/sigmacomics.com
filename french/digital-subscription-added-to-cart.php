...
/*if(!isset($_SESSION['add_to_cart_session_id']) && $_SESSION['add_to_cart_session_id']== ''){
	header( 'Location:'.constant('SITEURL_FRENCH').'/order-issues-2');
}*/
...
$sql = "SELECT SUM(quantity) as total_quantity,sum(price) as total_price FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' and `language` = 'fr_FR' ";
                ...
                <div class="new-add-cart-top-sec">
                    <ul>
                        ...
                        <li class="total-cart-itm">
                            <p><?php echo (isset($content[42]))?$content[42]:'';?> (<?php echo $total_quantity; ?> <?php echo (isset($content[46]))?$content[46]:'';?>): <?php echo calculateCurrencyAmount($countryCurrency,$countryCurrencyRate,$total_price);?></p>
                        </li>

                        <li class="buy-more-btn"><a href="<?php echo constant("SITEURL_FRENCH")."/cart";?>"><?php echo (isset($content[44]))?$content[44]:'';?></a></li>
                        <li class="proceed-checkout-btn"><a href="<?php echo constant("SITEURL_FRENCH")."/checkout";?>"><?php echo (isset($content[45]))?$content[45]:'';?> (<?php echo $total_quantity; ?> <?php echo (isset($content[46]))?$content[46]:'';?>)</a></li>
                    </ul>
                </div>
                ...
                <?php if(!empty($products['product'])){ ?>
                    ...
                    <div class="new-cart-dtl-itm">
                        ...
                        <article>
                            ...
                            <button class="cartbutton" onclick="addToCartIndividualFrench(<?php echo $_GET['product'];?>,<?php echo $products['product']['variants'][0]['price'];?>,'DS');">AJOUTER AU PANIER</button>
                        </article>
                    </div>
                <?php }else{
...
<script>
   ...
   // console.lo
    $('.new-cart-dtl-itm > article > h2 > div:nth-child(1) > span > strong').text(
        htmlPerIssuePriceText.replace(htmlPerIssuePriceText.split(' ')[0]+" "+htmlPerIssuePriceText.split(' ')[1],calculateCurrencyAmount(countryCurrency,countryCurrencyRate,htmlPerIssuePriceText.split(' ')[0].split('$')[1],countryInfo)));

    let htmlTopParagraphPriceText =  $('.new-cart-dtl-itm-hdn > h3').text();
    let replaceHtmlTopParagraphPriceText = htmlTopParagraphPriceText.split(' ')[27]+" "+htmlTopParagraphPriceText.split(' ')[28]+" "+htmlTopParagraphPriceText.split(' ')[29];
    $('.new-cart-dtl-itm-hdn > h3').text(
        htmlTopParagraphPriceText.replace(replaceHtmlTopParagraphPriceText,calculateCurrencyAmount(countryCurrency,countryCurrencyRate,htmlTopParagraphPriceText.split(' ')[27],countryInfo)));
</script>
</html>