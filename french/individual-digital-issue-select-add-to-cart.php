...
<body>
	...
	<section class="inner-page-body-sec news-page-body">
			...
			<?php if(!empty($products['product'])){ ?>
				...
				<div class="indi_list">
					<?php echo (isset($content[14]))?$content[14]:'';?>
				</div>
				...
				<div id="showProduct" >
					<div class="indidetailBody">
						...
						<h3><?php echo ( isset($products['product']['title']) )?$products['product']['title']:'';?> </h3>
							<div class="priceprt">
								<span>Prix de la liste: <del>USD<?php echo ( isset($products['product']['variants'][0]['compare_at_price']) )?$products['product']['variants'][0]['compare_at_price']:''; ?></del></span>
								<span>Prix : <b>USD<?php echo ( isset($products['product']['variants'][0]['price']) )?$products['product']['variants'][0]['price']:''; ?> !</b></span>
								<?php if( isset($products['product']['variants'][0]['compare_at_price']) && isset($products['product']['variants'][0]['price']) && ( intval($products['product']['variants'][0]['compare_at_price']) > intval($products['product']['variants'][0]['price']) ) ){ ?>
									<span>Vous économisez: <font>USD<?php echo $products['product']['variants'][0]['compare_at_price'] - $products['product']['variants'][0]['price'] ?></font></span>
								<?php }else{ ?> 
									<span>Vous économisez: <font>USD 0</font></span>
								<?php } ?>
								
							</div>
							<?php echo ( isset($products['product']['body_html']) )?$products['product']['body_html']:''; ?>
							
							<button class="cartbutton" onclick="addToCartIndividualFrench('<?php echo $_GET['product'];?>','<?php echo $products['product']['variants'][0]['price'];?>','IDI');">AJOUTER AU PANIER</button>
	...
	<script>
		/*$(document).ready(function(){
				$.ajax({ 
					url: SITEURL+"/manage/individual-digital-issue-select-add-to-cart-page-loaded.php",
					type: 'POST',
