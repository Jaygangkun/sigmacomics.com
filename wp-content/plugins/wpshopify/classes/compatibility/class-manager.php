...
class Manager
{
    ...
    public function delete_compatibility_mu()
    {
        ...
        if ($filesystem->file_exists($this->mu_plugin_dest) && !$removed) {
            error_log(
                'WP Shopify Pro Error: The compatibility plugin could not be deactivated because your mu-plugin directory is currently not writable.  Please update the permissions of the mu-plugins folder.'
            );
            return false;
        }
