...
if(!isset($_SESSION['add_to_cart_session_id']) && $_SESSION['add_to_cart_session_id']== ''){
    header( 'Location:'.constant('SITEURL_SPANISH').'/order-issues-2');
    exit();
}else{

    $sqlNew = "SELECT count(*) as total_digital FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' AND (`type`= 'IDI' OR `type`= 'DS' OR `type`= 'PRDS' OR `type`= 'PCDS') and `language` = 'es_ES'";
    if ($resultNew = $conn->query($sqlNew)) {
        $rowNew = $resultNew->fetch_assoc();
        if( ($rowNew['total_digital'] > 0) && ( !isset($_SESSION['user'])) ){
            header( 'Location:'.constant('SITEURL_SPANISH').'/signup');
            exit();
        }
    }


    $sqlPro = "SELECT SUM(quantity) as total_quantity,sum(price) as total_price FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' and `language` = 'es_ES'";
            ...
            if($charge->id != ''){
                ...
                if ($conn->query($sqlOrder) === TRUE) {
                    ...
                    if( in_array('IDI',$typesArr) || in_array('DS',$typesArr) || in_array('PRDS',$typesArr) || in_array('PCDS',$typesArr) ){
                        ...
                        header( 'Location:'.constant('SITEURL_SPANISH').'/thank-you-print-2');
                    }else{
                        header( 'Location:'.constant('SITEURL_SPANISH').'/thank-you-for-order-2');
                    }
...
?>
    ...
    <section class="inner-page-body-sec contact-page-body">
        <div class="container">
            <h2 class="text-center checkheading"><?php echo (isset($content[19]))?$content[19]:'';?> <span>( <?php echo $total_quantity;?> <?php echo($total_quantity > 1)?'Artículo':'Artículo'; ?> )</span></h2>
            <div class="cartWrap">
                ...
                <div class="leftcheckout">
                    <div class="express-checkoutprt">
                        ...
                        <b><p>PAGA RÁPIDO con PAYPAL!</p></b><br>
                        <div id="paypal-button-container"></div>
                        <div id="paypal-button" style="width: 100%"></div>
                    </div>
                    <br>
                    <div class="orprt">...o paga más lento con tarjeta de crédito</div><br>

                    <form action= "" method="post" id = "stripe_pay">
                        <div class="formPrt">
                            <span class="shipHeading"><b><?php echo (isset($content[20]))?$content[20]:'';?></b></span>
                            <div class="addressdetail">
                                <input name="first_name" id="first_name" type="text" placeholder="Nombre de pila">
                                <input name="last_name" id="last_name" type="text" placeholder="Apellido">
                                <input name="address1" id="address1" placeholder="Dirección">
                                <input name="address2" id="address2" placeholder="Dirección 2">
                                <input name="city" id="city" placeholder="Ciudad">
                                <input name="state" id="state" placeholder="Provincia del estado">
                                <input name="country" id="country" placeholder="País">
                                <input name="zip" id="zip" placeholder="Cremallera">
                            ...
                            <div class="addressdetail">
                                <div class="field-row">
                                    ...
                                    <input type="text" id="name"
                                           name="name" class="demoInputBox" placeholder="Nombre del titular de la tarjeta">
                                </div>
                                <div class="field-row">
                                    <!-- <span id="email-info" class="info"></span> -->
                                    <input type="text" id="email" name="email" class="demoInputBox" placeholder="Correo electrónico">
                                </div>
                                <div class="field-row full-width">
                                    <!-- <span id="card-number-info"
                                        class="info"></span> -->
                                    <input type="text" id="card-number"
                                           name="card-number" class="demoInputBox" placeholder="Número de tarjeta">
                                </div>
                                <div class="field-row full-width">
                        ...
                        <div class="reviewSection">
                            ...
                            <div class="delivery_box">
                                ...
                                <?php
                                $paypalProducts = '';
                                $sql = "SELECT * from `cart` WHERE `session_id` = '".$_SESSION['add_to_cart_session_id']."'  and `language` = 'es_ES'";
                                ...
                                if( !empty($result) ){
                                    ...
                                    <?php }else{
                                        header( 'Location:'.constant('SITEURL_SPANISH').'/order-issues');
                                    } }
                ...
                <div class="order-summeryBox">
                    ...
                    <div class="midboxOrder">
                        ...
                        <div class="itemList">
                            ...
                            <span>
									<label>Envío y manipulación</label>
                            ...
                            <div class="totalPrt">
                                ...
                                <span>
										<label>Total antes de los impuestos:*</label>
										<?php echo  (isset($countryCurrency->geoplugin_currencySymbol_UTF8)?$countryCurrency->geoplugin_currencySymbol_UTF8:"$")?>0.00
    ...
    <script>
        function cardValidation () {
            ...
            if(valid == false) {
                $("#error-message").html("TODOS LOS CAMPOS SON REQUERIDOS EXCEPTO LA DIRECCIÓN 2").show();
            }

            if(invalidEmail == false) {
                $("#error-message-individual").html("Email inválido. Utilice el formato correcto").show();
            }else{
                $("#error-message-individual").html("").hide();
            }

            if(invalidCard == false) {
                $("#error-message-individual").html("Tarjeta no valida. Utilice uno correcto").show();
            }else{
                $("#error-message-individual").html("").hide();
            }
            ...
            onAuthorize: function (data, actions) {
                return actions.payment.execute()
                    .then(function (details) {
                        console.log(details);
                        window.location = "<?php echo constant("SITEURL_SPANISH");?>/orderDetails.php?paymentID="+
