...
if(isset($_SESSION['user']) && $_SESSION['user']['id']!= ''){
	header( 'Location:'.constant('SITEURL_SPANISH').'/digital-dashboard');
}
...
if(isset($_REQUEST['login_next'])){
	...
	header( 'Location:'.constant('SITEURL_SPANISH').'/login2');
	exit();
}

/*$sqlNew = "SELECT `post_content` FROM `wp_posts` where `post_type`= 'custom-field-spanish' and `post_status`= 'publish' ";*/

/*$sqlNew = "SELECT * FROM wp_posts LEFT JOIN wp_term_relationships ON (wp_posts.ID = wp_term_relationships.object_id) LEFT JOIN wp_term_taxonomy ON (wp_term_relationships.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id) WHERE wp_posts.post_type = 'custom-field' and wp_posts.post_status= 'publish' AND wp_term_taxonomy.term_id = 48 ORDER BY `ID` ASC ";
		...
		<div class="container">
			<div class="loginWrap">
				<div class="toploginBox">
					...
					<button class="redbutton" name="login_next"><?php echo (isset($content[37]))?$content[37]:'';?></button>
					</form>
