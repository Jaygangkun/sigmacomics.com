...
$sql = "SELECT SUM(quantity) as total_quantity,sum(price) as total_price FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' and `language` = 'es_ES' ";
            ...
            <div class="container">
                <div class="new-add-cart-top-sec">
                    <ul>
                        ...
                        <li class="buy-more-btn"><a href="<?php echo constant("SITEURL_SPANISH")."/cart";?>"><?php echo (isset($content[44]))?$content[44]:'';?></a></li>
                        <li class="proceed-checkout-btn"><a href="<?php echo constant("SITEURL_SPANISH")."/checkout";?>"><?php echo (isset($content[45]))?$content[45]:'';?> (<?php echo $total_quantity; ?> <?php echo (isset($content[46]))?$content[46]:'';?>)</a></li>
                    </ul>
                </div>
                ...
                <?php if(!empty($products['product'])){ ?>
                    ...
                    <div class="new-cart-dtl-itm">
                        ...
                        <article>
                            ...
                            <button class="cartbutton" onclick="addToCartIndividualSpanish(<?php echo $_GET['product'];?>,<?php echo $products['product']['variants'][0]['price'];?>,'DS');">AÑADIR AL CARRITO</button>
                        </article>
                    </div>
                <?php }else{
...
<script>
    ...
    $('.new-cart-dtl-itm > article > h2 > div:nth-child(1) > span > strong').text(
        htmlPerIssuePriceText.replace(htmlPerIssuePriceText.split(' ')[1]+" "+htmlPerIssuePriceText.split(' ')[2],calculateCurrencyAmount(countryCurrency,countryCurrencyRate,htmlPerIssuePriceText.split(' ')[2].split(',')[0]+"."+htmlPerIssuePriceText.split(' ')[2].split(',')[1],countryInfo)));
</script>
</html>