...
include_once('wp-content/themes/sigma-comics/apis.php');
...
if(!isset($_SESSION['add_to_cart_session_id']) && $_SESSION['add_to_cart_session_id']== ''){
	...
	$sqlNew = "SELECT count(*) as total_digital FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' AND (`type`= 'IDI' OR `type`= 'DS' OR `type`= 'PRDS' OR `type`= 'PCDS') and `language` = 'en_US'";
	...
	$sqlPro = "SELECT SUM(quantity) as total_quantity,sum(price) as total_price FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' and `language` = 'en_US' ";
			...
			if($charge->id != ''){
				...
				if ($conn->query($sqlOrder) === TRUE) {
					...
					if( in_array('IDI',$typesArr) || in_array('DS',$typesArr) || in_array('PRDS',$typesArr) || in_array('PCDS',$typesArr) ){
						...
						header( 'Location: thank-you-print');
					}else{
...
<body>
	...
	<section class="inner-page-body-sec contact-page-body">
		<div class="container">
			<h2 class="text-center checkheading"><?php echo (isset($content[19]))?$content[19]:'';?> <span>( <?php echo $total_quantity;?> <?php echo($total_quantity > 1)?'items':'item'; ?> )</span></h2>
			<div class="cartWrap">
				...
				<form action= "" method="post" id = "stripe_pay">
					<div class="formPrt">
						...
						<input name="address1" id="address1" placeholder="Address">	
						<input name="address2" id="address2" placeholder="Address 2">
...
function cardValidation () {
    ...
    if(valid == false) {
        $("#error-message").html("ALL FIELDS ARE REQUIRED EXCEPT ADDRESS 2").show();
    }
			...
			paypal.Button.render({
				...
				onAuthorize: function (data, actions) {
					...
					.then(function (details) {
						console.log(details);
						window.location = "<?php echo constant("SITEURL");?>/orderDetails.php?paymentID="+
