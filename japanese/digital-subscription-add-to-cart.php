...
// product data
$data = array("collection_id" => '262392873125');
...
?>
                    ...
                    <?php if(!empty($products['product'])){ ?>
                        ...
                        <div class="indidetailBody">
                            ...
                            <div class="rightContentTxt">
                                ...
                                <div class="priceprt">
                                    <span>定価: <del><?php echo ( isset($products['product']['variants'][0]['compare_at_price']) )?calculateCurrencyAmount($countryCurrency,$countryCurrencyRate,$products['product']['variants'][0]['compare_at_price']):''; ?></del></span>
                                    <span>価格: <b><?php echo ( isset($products['product']['variants'][0]['price']) )?calculateCurrencyAmount($countryCurrency,$countryCurrencyRate,$products['product']['variants'][0]['price']):''; ?> !</b></span>
                                    <?php if( isset($products['product']['variants'][0]['compare_at_price']) && isset($products['product']['variants'][0]['price']) && ( intval($products['product']['variants'][0]['compare_at_price']) > intval($products['product']['variants'][0]['price']) ) ){ ?>
                                        <span>節約: <font><?php echo calculateCurrencyAmount($countryCurrency,$countryCurrencyRate,$products['product']['variants'][0]['compare_at_price'] - $products['product']['variants'][0]['price']) ?></font></span>
                                    <?php }else{ ?>
                                        <span>節約: <font>$0</font></span>
                                    <?php } ?>

                                </div>
                                <?php echo ( isset($products['product']['body_html']) )?$products['product']['body_html']:''; ?>

                                <button class="cartbutton" onclick="addToCartIndividualJapanese(<?php echo $products['product']['id'];?>,<?php echo $products['product']['variants'][0]['price'];?>,'DS');">カートに追加</button>
    ...
    <script>
        ...
        $('.indi_list > h4')
            .text(htmlShortHeadingText.replace(htmlShortHeadingArray[1]+" "+htmlShortHeadingArray[2],calculateCurrencyAmount(countryCurrency,countryCurrencyRate,htmlShortHeadingArray[1],countryInfo)));
        $('.indi_list > h4').text($('.indi_list > h4').text().replace('$'," "));
        ...
        $('.rightContentTxt > div:nth-child(3) > span > strong')
            .text(htmlShortHeadingText2.replace(htmlShortHeadingArray2[1] +" "+htmlShortHeadingArray[2],calculateCurrencyAmount(countryCurrency,countryCurrencyRate,htmlShortHeadingArray[1],countryInfo)));
        $('.rightContentTxt > div:nth-child(3) > span > strong').text($('.rightContentTxt > div:nth-child(3) > span > strong').text().replace('$'," "));
    </script>
    </html>
<?php die();?>