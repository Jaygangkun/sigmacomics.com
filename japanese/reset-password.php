			...
			if ($result = $conn->query($sql)) {
				...
				if($row['total'] > 0){
					...
					if ($conn->query($sql) === TRUE) {
						header( 'Location:'.constant('SITEURL_JAPANESE').'/reset-password-2.php');
					}
				}
			}else {
				$_SESSION['message'] =  "Error: " . $sql . "<br>" . $conn->error;
				header( 'Location:'.constant('SITEURL_JAPANESE').'/forgot-password');
				exit();
			}
		}
	}
			...
			<div class="loginWrap">
				<div class="toploginBox">
					...
					<div class="form-group">
						<p class="bigTxt">アカウントでパスワードを入力します。</p>
					</div>
					<div class="form-group">
						<div class="labelwrap">
						...
						<div class="labelwrap">
							<label>パスワードを確認する</label>
						</div>

						<input type="password" name="confirmpassword"  class="form-control">
					</div>
					<button class="getbutton"  name="forgot_pass_next">送信するには </button>
					</form>
					<div class="checkboxprt">
					まだ問題がありますか？ <a href="<?php echo constant("SITEURL_JAPANESE")."/contact";?>">お問い合わせ.</a>
					</div>
