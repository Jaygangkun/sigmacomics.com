...
if (isset($_SESSION['add_to_cart_session_id']) && $_SESSION['add_to_cart_session_id'] != '' ){
	$sql = "SELECT * from `cart` WHERE `session_id` = '".$_SESSION['add_to_cart_session_id']."' and `language` = 'en_US' ";
	$result = $conn->query($sql);

	$sqlNew = "SELECT SUM(quantity) as total_quantity,sum(price) as total_price FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' and `language` = 'en_US' ";
		...
		<div class="container">
			<div class="cartWrap">
									...
									<div class="productSHowprt">
										...
										<div class="productCont">
											...
											<div class="quantity_prt">
												<div class="leftqty">
												<?php echo utf8_encode((isset($content[47]))?$content[47]:'');?>:
												...
												<a href="javascript:void(0);" onclick="cartProductDelete(<?php echo $productId;?>)"><?php echo utf8_encode((isset($content[48]))?$content[48]:'');?></a>
				...
				<div class="rightcrtprice">
					<h4><?php echo (isset($content[18]))?$content[18]:'';?>(<?php echo ( isset($total_quantity) && $total_quantity != '' )?$total_quantity:'0'; ?> <?php echo ( isset($total_quantity) && $total_quantity != '' && $total_quantity > '1' )?'items':'item'; ?>): <b><?php echo ( isset($total_price) && $total_price != '' )?calculateCurrencyAmount($countryCurrency,$countryCurrencyRate,$total_price):'0.00'; ?> </b></h4>
					<a href="<?php echo constant("SITEURL")?>/checkout"><button class="redbutton">
					<?php echo utf8_encode((isset($content[49]))?$content[49]:'');?>  ( <?php echo ( isset($total_quantity) && $total_quantity != '' )?$total_quantity:'0'; ?> <?php echo ( isset($total_quantity) && $total_quantity != '' && $total_quantity > '1' )?'items':'item'; ?>)
					</button></a>
				</div>
			</div>
