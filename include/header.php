...
/*$sqlAll = "SELECT `post_content` FROM `wp_posts` where `post_type`= 'custom-field' and `post_status`= 'publish' ";*/

$sqlAll = "SELECT * FROM wp_posts LEFT JOIN wp_term_relationships ON (wp_posts.ID = wp_term_relationships.object_id) LEFT JOIN wp_term_taxonomy ON (wp_term_relationships.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id) WHERE wp_posts.post_type = 'custom-field' and wp_posts.post_status= 'publish' AND wp_term_taxonomy.term_id = 1 ORDER BY `ID` ASC ";
				...
				<div class="logo-hdr-row">
					...
					<div class="hdr-login-btn">
						...
						if( isset($_SERVER) ){
							...
							$param = explode(constant("SITEURL"),$actual_link);
						?>

						<?php
						}
						?>
						<!--<form method='post' action='languageSubmit.php' id='form_lang' >
