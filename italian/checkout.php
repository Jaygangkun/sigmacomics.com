...
if(!isset($_SESSION['add_to_cart_session_id']) && $_SESSION['add_to_cart_session_id']== ''){
    ...
    $sqlNew = "SELECT count(*) as total_digital FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' AND (`type`= 'IDI' OR `type`= 'DS' OR `type`= 'PRDS' OR `type`= 'PCDS') and `language` = 'it_IT'";
    if ($resultNew = $conn->query($sqlNew)) {
        $rowNew = $resultNew->fetch_assoc();
        if( ($rowNew['total_digital'] > 0) && ( !isset($_SESSION['user'])) ){
            header( 'Location:'.constant('SITEURL_ITALIAN').'/signup');
            exit();
        }
    }


    $sqlPro = "SELECT SUM(quantity) as total_quantity,sum(price) as total_price FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' and `language` = 'it_IT'";
            ...
            if($charge->id != ''){
                ...
                if ($conn->query($sqlOrder) === TRUE) {
                    ...
                    if( in_array('IDI',$typesArr) || in_array('DS',$typesArr) || in_array('PRDS',$typesArr) || in_array('PCDS',$typesArr) ){
                        ...
                        header( 'Location:'.constant('SITEURL_ITALIAN').'/thank-you-print-2');
                    }else{
                        header( 'Location:'.constant('SITEURL_ITALIAN').'/thank-you-for-order-2');
                    }
...
?>
    ...
    <section class="inner-page-body-sec contact-page-body">
        <div class="container">
            <h2 class="text-center checkheading"><?php echo (isset($content[19]))?$content[19]:'';?> <span>( <?php echo $total_quantity;?> <?php echo($total_quantity > 1)?'Articolo':'Articolo'; ?> )</span></h2>
            <div class="cartWrap">


                <div class="leftcheckout">
                    <div class="express-checkoutprt">
                        <b><p>PAGA VELOCEMENTE CON PAYPAL</p></b><br>

                        <div id="paypal-button-container"></div>
                        <div id="paypal-button" style="width: 100%"></div>
                    </div>

                    <div class="orprt">...o paga più lentamente con una carta di credito</div><br>

                    <form action= "" method="post" id = "stripe_pay">
                        <div class="formPrt">
                            <span class="shipHeading"><b><?php echo (isset($content[20]))?$content[20]:'';?></b></span>
                            <div class="addressdetail">
                                <input name="first_name" id="first_name" type="text" placeholder="nome di battesimo">
                                <input name="last_name" id="last_name" type="text" placeholder="Cognome">
                                <input name="address1" id="address1" placeholder="Indirizzo 1">
                                <input name="address2" id="address2" placeholder="Indirizzo 2">
                                <input name="city" id="city" placeholder="città">
                                <input name="state" id="state" placeholder="Stato">
                                <input name="country" id="country" placeholder="nazione">
                                <input name="zip" id="zip" placeholder="Codice postale">
                            ...
                            <div class="addressdetail">
                                <div class="field-row">
                                    ...
                                    <input type="text" id="name"
                                           name="name" class="demoInputBox" placeholder="Cognome">
                                </div>
                                <div class="field-row">
                                    <!-- <span id="email-info" class="info"></span> -->
                                    <input type="text" id="email" name="email" class="demoInputBox" placeholder="e-mail">
                                </div>
                                <div class="field-row full-width">
                                    <!-- <span id="card-number-info"
                                        class="info"></span> -->
                                    <input type="text" id="card-number"
                                           name="card-number" class="demoInputBox" placeholder="Numero di carta">
                                </div>
                                <div class="field-row full-width">
                                    <div class="contact-row column-right">
                                        <label>Scadenza mese / anno</label>
                        ...
                        <div class="reviewSection">
                            ...
                            <div class="delivery_box">
                                ...
                                <?php
                                $paypalProducts = '';
                                $sql = "SELECT * from `cart` WHERE `session_id` = '".$_SESSION['add_to_cart_session_id']."'  and `language` = 'it_IT'";
                                ...
                                if( !empty($result) ){
                                    ...
                                    <?php }else{
                                        header( 'Location:'.constant('SITEURL_ITALIAN').'/order-issues');
                                    } }
    ...
    <script>
        function cardValidation () {
            ...
            if(valid == false) {
                $("#error-message").html("TUTTI I CAMPI SONO OBBLIGATORI TRANNE L'INDIRIZZO 2").show();
            }

            if(invalidEmail == false) {
                $("#error-message-individual").html("E-mail non valido. Si prega di utilizzare il formato corretto").show();
            }else{
                $("#error-message-individual").html("").hide();
            }

            if(invalidCard == false) {
                $("#error-message-individual").html("Carta non valida. Si prega di usarne uno corretto").show();
            }else{
                $("#error-message-individual").html("").hide();
            }
            ...
            onAuthorize: function (data, actions) {
                return actions.payment.execute()
                    .then(function (details) {
                        console.log(details);
                        window.location = "<?php echo constant("SITEURL_ITALIAN");?>/orderDetails.php?paymentID="+
