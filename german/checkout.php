...
if(!isset($_SESSION['add_to_cart_session_id']) && $_SESSION['add_to_cart_session_id']== ''){
    header( 'Location:'.constant('SITEURL_GERMAN').'/order-issues-4');
    exit();
}else{

    $sqlNew = "SELECT count(*) as total_digital FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' AND (`type`= 'IDI' OR `type`= 'DS' OR `type`= 'PRDS' OR `type`= 'PCDS') and `language` = 'de_DE'";
    if ($resultNew = $conn->query($sqlNew)) {
        $rowNew = $resultNew->fetch_assoc();
        if( ($rowNew['total_digital'] > 0) && ( !isset($_SESSION['user'])) ){
            header( 'Location:'.constant('SITEURL_GERMAN').'/signup');
            exit();
        }
    }


    $sqlPro = "SELECT SUM(quantity) as total_quantity,sum(price) as total_price FROM `cart` where `session_id`= '".$_SESSION['add_to_cart_session_id']."' and `language` = 'de_DE'";
            ...
            if($charge->id != ''){
                ...
                if ($conn->query($sqlOrder) === TRUE) {
                    ...
                    if( in_array('IDI',$typesArr) || in_array('DS',$typesArr) || in_array('PRDS',$typesArr) || in_array('PCDS',$typesArr) ){
                        ...
                        header( 'Location:'.constant('SITEURL_GERMAN').'/thank-you-print-2');
                    }else{
                        header( 'Location:'.constant('SITEURL_GERMAN').'/thank-you-for-order-2');
                    }
...
?>
            ...
            <div class="cartWrap">
                ...
                <div class="leftcheckout">
                    <div class="express-checkoutprt">
                        <b><p>SCHNELL MIT PAYPAL ZAHLEN!</p></b><br>
                        <div id="paypal-button-container"></div>
                        <div id="paypal-button" style="width: 100%"></div>
                    </div>

                    <div class="orprt">... oder langsamer mit Kreditkarte bezahlen</div>
                    ...
                    <form action= "" method="post" id = "stripe_pay">
                        <div class="formPrt">
                            ...
                            <div class="addressdetail">
                                <input name="first_name" id="first_name" type="text" placeholder="Vorname">
                                <input name="last_name" id="last_name" type="text" placeholder="Nachname">
                                <input name="address1" id="address1" placeholder="Adresse 1">
                                <input name="address2" id="address2" placeholder="Adresse 2">
                                <input name="city" id="city" placeholder="Stadt">
                                <input name="state" id="state" placeholder="Zustand">
                                <input name="country" id="country" placeholder="Land">
                                <input name="zip" id="zip" placeholder="Postleitzahl">
                            ...
                            <div class="addressdetail">
                                <div class="field-row">
                                    ...
                                    <input type="text" id="name"
                                           name="name" class="demoInputBox" placeholder="Name">
                                </div>
                                <div class="field-row">
                                ...
                                <div class="field-row full-width">
                                    ...
                                    <input type="text" id="card-number"
                                           name="card-number" class="demoInputBox" placeholder="Kartennummer">
                                </div>
                                <div class="field-row full-width">
                                    <div class="contact-row column-right">
                                        <label>Ablaufmonat / -jahr</label>
                        ...
                        <div class="reviewSection">
                            ...
                            <div class="delivery_box">
                                ...
                                if( !empty($result) ){
                                    ...
                                    <?php }else{
                                        header( 'Location:'.constant('SITEURL_GERMAN').'/order-issues');
                                    } }
    ...
    <script>
        function cardValidation () {
            ...
            if(valid == false) {
                $("#error-message").html("ALLE FELDER SIND ERFORDERLICH, MIT AUSNAHME DER ADRESSE 2").show();
            }

            if(invalidEmail == false) {
                $("#error-message-individual").html("Ungültige E-Mail. Bitte verwenden Sie das richtige Format").show();
            }else{
                $("#error-message-individual").html("").hide();
            }

            if(invalidCard == false) {
                $("#error-message-individual").html("Ungültige Karte. Bitte verwenden Sie die richtige").show();
            }else{
                $("#error-message-individual").html("").hide();
            }
            ...
            onAuthorize: function (data, actions) {
                return actions.payment.execute()
                    .then(function (details) {
                        console.log(details);
                        window.location = "<?php echo constant("SITEURL_GERMAN");?>/orderDetails.php?paymentID="+
