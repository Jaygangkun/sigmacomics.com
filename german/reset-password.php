			...
			if ($result = $conn->query($sql)) {
				...
				if($row['total'] > 0){
					...
					if ($conn->query($sql) === TRUE) {
						header( 'Location:'.constant('SITEURL_GERMAN').'/reset-password-2.php');
					}
				}
			}else {
				$_SESSION['message'] =  "Error: " . $sql . "<br>" . $conn->error;
				header( 'Location:'.constant('SITEURL_GERMAN').'/forgot-password');
				exit();
			}
		}
	}
			...
			<div class="loginWrap">
				<div class="toploginBox">
					...
					<div class="form-group">
						<p class="bigTxt">Geben Sie das Passwort mit Ihrem Konto ein.</p>
					</div>
					<div class="form-group">
						<div class="labelwrap">
					...
					<button class="getbutton"  name="forgot_pass_next">Einreichen </button>
					</form>
					<div class="checkboxprt">
					Immer noch Probleme? Póngase en <a href="<?php echo constant("SITEURL_GERMAN")."/contact";?>">In Verbindung treten</a>
					</div>
