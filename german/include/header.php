...
/*if(isset($_SESSION['lang']) && $_SESSION['lang'] == 'es_ES' ) {
	header('Location: '.constant("SITEURL_GERMAN").$_SERVER['PHP_SELF']);
	exit();
}*/

/*$sqlAll = "SELECT `post_content` FROM `wp_posts` where `post_type`= 'custom-field-spanish' and `post_status`= 'publish' ";*/

$sqlAll = "SELECT * FROM wp_posts LEFT JOIN wp_term_relationships ON (wp_posts.ID = wp_term_relationships.object_id) LEFT JOIN wp_term_taxonomy ON (wp_term_relationships.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id) WHERE wp_posts.post_type = 'custom-field' and wp_posts.post_status= 'publish' AND wp_term_taxonomy.term_id = 78 ORDER BY `ID` ASC ";
				...
				<div class="logo-hdr-row">
					...
					<div class="hdr-login-btn">
						<?php if( isset($_SESSION['user'])){ ?>  
							...
							<div class="hdr-login"><a href="<?php echo constant("SITEURL_GERMAN")."/logout.php";?> ">Logout</a></div>
						<?php }else{ ?> 
							<div class="hdr-login"><a href="<?php echo constant("SITEURL_GERMAN")."/login";?>">Login</a></div>
						<?php } ?>
						<?php 
						
						if( isset($_SERVER) ){
							$actual_link = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
							$param = explode(constant("SITEURL_GERMAN"),$actual_link);
						?>
