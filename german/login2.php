...
$sqlNew = "SELECT * FROM wp_posts LEFT JOIN wp_term_relationships ON (wp_posts.ID = wp_term_relationships.object_id) LEFT JOIN wp_term_taxonomy ON (wp_term_relationships.term_taxonomy_id = wp_term_taxonomy.term_taxonomy_id) WHERE wp_posts.post_type = 'custom-field' and wp_posts.post_status= 'publish' AND wp_term_taxonomy.term_id = 78 ORDER BY `ID` ASC ";
	...
	if( $user_password != '' && isset($_SESSION['user_email_login2']) ){
		...
		if ($result = $conn->query($sql)) {
			...
			if($row == 1 ){
				...
				// 	header( 'Location:'.constant('SITEURL_GERMAN').'/cart');
				// }else{
					header( 'Location:'.constant('SITEURL_GERMAN').'/digital-dashboard-4');
				}
				exit();
